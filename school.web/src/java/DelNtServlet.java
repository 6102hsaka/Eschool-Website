 ...
 * @author TT
 */
@WebServlet(urlPatterns = {"/DelNtServlet"})
public class DelNtServlet extends HttpServlet {
        ...
        try
            {
                ...
                String str="delete from notice where Description=?";
                pst=cn.prepareStatement(str);
                pst.setString(1, sub);
                res = pst.executeUpdate();
                if(res==1)
                {
                    ses.setAttribute("msg1", "Record Deleted");
                    rd =request.getRequestDispatcher("/alterNotice.jsp");
                    rd.forward(request, response);
                }
                else
                {
                    ses.setAttribute("msg1", "Error Occured!!!!");
                    rd =request.getRequestDispatcher("/alterNotice.jsp");
                    rd.forward(request, response);   
                }
            }
